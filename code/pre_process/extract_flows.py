...
def extracting_flows(dataset, dataset_name, of_save_dir):
    ...
    for idx, (batch, _) in tqdm(enumerate(dataset_loader), total=len(dataset)):
        ...
        # Pfad zum 'image'-Ordner, basierend auf `cur_img_addr` und `of_save_dir`
...
if __name__ == '__main__':
    ...
    parser.add_argument("--proj_root", type=str, default="/disk/vanishing_data/hb344/", help='project root path')
    ...
    if args.mode == "train":
        of_save_dir = os.path.join(args.proj_root, 'flows_carla', "training")
    else:
        of_save_dir = os.path.join(args.proj_root, 'flows_carla', "testing")
