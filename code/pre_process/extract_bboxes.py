...
"carla_local": {
    ...
    "binary_thr": 25,  
    "gauss_mask_size": 3, 
    "contour_min_area": 30 * 30
}
}

task = Task.init(project_name='bogdoll/anomaly_detection_hybrid', task_name='extract_flows')
...
def obj_bboxes_extraction(dataset_root, dataset_name, mode):
    ...
    dataset_root = '/disk/users/hb344/no_backup/Desktop/hf2vad/carla_data/bboxes_carla'
    
    np.save(os.path.join(os.path.join(dataset_root, dataset_name),
                         '%s_bboxes_%s.npy' % (dataset_name, mode)), all_bboxes)
    print('bboxes saved!')

    np.save(os.path.join(dataset_root,'%s_bboxes_%s.npy' % (dataset_name, mode)), all_bboxes)
    print('bboxes saved!')
